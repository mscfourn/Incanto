define(["esri/map",
		...
		Helper)
	{
		...
		var timeSlider = null;
	    function initSlider(map) {
	    	if (!timeSlider) {
        	 	timeSlider = new esri.dijit.TimeSlider({style: "width: 100%;"}, dojo.byId("timeSliderDiv"));
	        	var timeExtent = new esri.TimeExtent();
	        	var incantiYears = [1294, 1301, 1310, 1312, 1320, 1338, 1350, 1374, 1375, 1389, 1408, 1412, 1419, 1437, 1441, 1444, 1464];
	        	var _timeStepIntervals = [];
	        	for (var i=0; i<incantiYears.length; i++) {
	        		_timeStepIntervals.push(new Date(incantiYears[i], 1, 1));
	        	}
				timeSlider.setTimeStops(_timeStepIntervals);
	        	timeSlider.startup();

				timeSlider.setLabels(incantiYears);
        	}
        	map.setTimeSlider(timeSlider);

/*
			var mapLayer = new esri.layers.ArcGISDynamicMapServiceLayer("");

			mapLayer.setVisibleLayers([0]);
			setLayerDefinitions(incantiYears[0], 1);
			map.addLayers([mapLayer]);
*/
			dojo.connect(timeSlider, "onTimeExtentChange", function(timeExtent) {
				var selectedYear = timeExtent.endTime.getUTCFullYear();
		...
		function loadMaps() {
			...
			mapDeferred.addCallback(function(response){
				...
				dojo.connect(map,"onExtentChange",function(){
					console.log('map.onExtentChange');
